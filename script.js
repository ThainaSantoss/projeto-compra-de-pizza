...
c(".pizzaInfo--addButton").addEventListener("click", () => {
  // qual tamanho? quantas foram e qual a pizza
  let size = parseInt(c(".pizzaInfo--size.selected").getAttribute("data-key"));
  let identifier = pizzaJson[modalKey].id + "@" + size;
  let key = cart.findIndex((item) => item.identifier == identifier); // se eu já tenho o item no carinho
  if (key > -1) {
    cart[key].qt += modalQt;
  } else {
    cart.push({
...
function updateCart() {
  ...
  if (cart.length > 0) {
    c("aside").classList.add("show"); // mostrando carinho
    c(".cart").innerHTML = "";
    ...
    for (let i in cart) {
      // mapear
      let pizzaItem = pizzaJson.find((item) => item.id == cart[i].id); // retorna *todos* os itens da pizza
